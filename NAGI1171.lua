...
getgenv().FourHub_set_antidetect = function(v)
...
local CUSTOM_UI_FONTS = {
    ...
    ['Tahoma'] = {
        ...
        AltFontFiles = { 'FourHub/Tahoma.font', 'library/Tahoma.font', 'Tahoma.ttf', 'library/Tahoma.ttf' },
    },
};
...
local Library = {
    ...
    AccentColor = Color3.fromHex("001EFF");
    OutlineColor = Color3.new(0, 0, 0);
    SelectedTabColor = Color3.fromHex("0a0a0a");
    GradientColor = Color3.fromHex("ffffff");
    ...
    OverlayGlowColor = Color3.fromHex('001EFF');
...
function Library:SetGradientText(Label, Text)
    ...
    local gradient = Label:FindFirstChild('FourHubTextGradient');
    if not gradient then
        gradient = Library:Create('UIGradient', {
            Name = 'FourHubTextGradient';
            Parent = Label;
        });
...
function Library:UpdateMenuBlur()
    ...
    local blur = lighting:FindFirstChild('FourHubMenuBlur');
...
getgenv().FourHub_wm = getgenv().FourHub_wm or { ping = true, executor = false, fps = true, time = true };
task.spawn(function()
    local fps = 60;
    local execName = nil;
    ...
    while task.wait(0.5) do
        if Library.Watermark and Library.Watermark.Visible then
            local wm = getgenv().FourHub_wm or {};
            local parts = { "VeloxHubRivals" };
...
function Library:SaveSoundSettings()
    ...
    local SettingsPath = (self.SoundSettingsFolder or 'FourHub') .. '/settings/sounds.json';
    local Data = {
...
function Library:InitSoundSettings(Folder)
    ...
    self.SoundSettingsFolder = Folder or 'FourHub';
...
function Library:InitSoundAssets()
    local Folder = self.SoundSettingsFolder or 'FourHub';
    ...
    if not self.ToggleSound then
        ...
        if ToggleAsset then
            ...
            self.ToggleSound.Name = 'FourHubToggleSound';
    ...
    if not self.NotificationSound then
        ...
        if NotifyAsset then
            ...
            self.NotificationSound.Name = 'FourHubNotificationSound';
    ...
    if not self.SliderSound then
        ...
        if SliderAsset then
            ...
            self.SliderSound.Name = 'FourHubSliderSound';
...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(520, 430) end

    do
        local saved = getgenv().FourHub_ui_size;
        if not saved then
            pcall(function()
                if isfile and isfile('FourHub/ui_size.txt') then
                    local d = tostring(readfile('FourHub/ui_size.txt'));
    ...
    do
        ...
        local function persistSize()
            ...
            getgenv().FourHub_ui_size = { math.floor(s.X), math.floor(s.Y) };
            pcall(function()
                if writefile then
                    if makefolder and isfolder and not isfolder('FourHub') then makefolder('FourHub') end;
                    writefile('FourHub/ui_size.txt', math.floor(s.X) .. ',' .. math.floor(s.Y));
                end;
            end);
        end;
    ...
    local function UpdateTitle()
        local rawTitle = Config.Title or "VeloxHubRivals";
    ...
    Library:Create('UICorner', { CornerRadius = UDim.new(0, 10); Parent = MainSectionInner });
    ...
    local SidebarSubtitle = Library:CreateLabel({
    ...
    local TabListLayout = Library:Create('UIListLayout', {
        Padding = UDim.new(0, Window.TabPadding);
        FillDirection = Enum.FillDirection.Vertical;
        SortOrder = Enum.SortOrder.LayoutOrder;
        Parent = TabArea;
    });
    ...
    function Window:UpdateTabLayout()
        ...
        local AreaHeight = self.TabArea.AbsoluteSize.Y;
        if AreaWidth <= 0 or AreaHeight <= 0 then
            return
        end;

        local Padding = self.TabPadding or 4;
        local SidePadding = 10;
        local BaseTextSize = self.TabTextSize or 12;
        local MinTextSize = 9;

        local TotalPadding = Padding * (Count - 1);
        local EqualHeight = math.max(22, math.floor((AreaHeight - TotalPadding) / Count));
        local AvailableTextWidth = AreaWidth - SidePadding * 2;
        ...
        for _, Entry in self.TabEntries do
            ...
            Entry.Button.Size = UDim2.new(1, 0, 0, EqualHeight);
        end;
    end;
    ...
    local TabContainer = Library:Create('Frame', {
        ...
        BorderColor3 = Library.OutlineColor;
        ClipsDescendants = true;
        Position = UDim2.new(0, 166, 0, 10);
        Size = UDim2.new(1, -176, 1, -20);
        ZIndex = 3;
        Parent = MainSectionInner;
    });
    ...
    Library:Create('UICorner', { CornerRadius = UDim.new(0, 10); Parent = TabContainer });
    ...
    function Window:AddTab(Name)
        ...
        Library:Create('UICorner', { CornerRadius = UDim.new(0, 7); Parent = TabButton });
...
local ThemeManager = {} do
	...
	ThemeManager.DefaultTheme = 'VeloxHubRivals';
	ThemeManager.BuiltInThemes = {
		...
		['FourHub.bloom'] 	= { 28, httpService:JSONDecode('{"FontColor":"3d3034","MainColor":"fff5f6","AccentColor":"ffa3b1","SelectedTabColor":"ebd5d8","BackgroundColor":"fff5f6","OutlineColor":"ffd6db","GradientColor":"e68190"}') },
		...
		['VeloxHubRivals'] = { 65, httpService:JSONDecode('{"FontColor":"e7ebf4","MainColor":"0d1017","AccentColor":"4f7cff","SelectedTabColor":"171d2a","BackgroundColor":"0a0d13","OutlineColor":"202838","GradientColor":"263b73"}') },
		['FourHub']         = { 65, httpService:JSONDecode('{"FontColor":"e0d4d6","MainColor":"121212","AccentColor":"001EFF","SelectedTabColor":"0a0a0a","BackgroundColor":"0a0a0a","OutlineColor":"000000","GradientColor":"6b3038"}') },
	};
	...
	function ThemeManager:LoadDefault()
		local theme = self.DefaultTheme or 'VeloxHubRivals';
	...
	function ThemeManager:CreateAppearanceManager(groupbox, skipLoadDefault)
		...
		groupbox:AddDropdown('ThemeManager_ThemeList', { Text = 'Presets', Values = ThemesArray, Default = 'FourHub' });
...
local SaveManager = (function()
    ...
    SaveManager.Folder = 'FourHub/rivals';
    SaveManager.Ignore = {};
    ...
    function SaveManager:Save(name)
        ...
        do
            data.widgets = {};
            local ws = getgenv().FourHub_widgets;
...
local Window = Library:CreateWindow({
    ...
    Resizable = true,
    Draggable = true,
    Size = ((game:GetService('UserInputService').TouchEnabled and not game:GetService('UserInputService').MouseEnabled) and UDim2.fromOffset(420, 360)) or UDim2.fromOffset(820, 680),
});
...
task.spawn(function()
    for name, suffix in font_files do
        local ttf_path = "FourHub/rivals/fonts/" .. suffix;
        local font_path = "FourHub/rivals/fonts/" .. name .. ".font";
...
getgenv().FourHub = {
    ...
    silent_aim = {
        enable = false,
        visualize = false,
        visualize_color = Color3.fromRGB(0, 30, 255),
        closest_part = false,
        show_fov = false,
        show_fill = false,
        fov_radius = 180,
        ...
        fov_color = Color3.fromRGB(0, 30, 255),
        fov_fill_color = Color3.fromRGB(0, 30, 255),
    ...
    esp = {
        ...
        highlight = {
            enable = false,
            fill_color = Color3.fromRGB(0, 30, 255),
            outline_color = Color3.fromRGB(255, 255, 255),
        ...
        text = {
            ...
            size = 9,
            gradient = false,
            gradient_color1 = Color3.fromRGB(0, 30, 255),
...
do
    ...
    getgenv().FourHub_apply_opt = function(kind)
    ...
    local FFLAG_PRESET = [==[
{
  ...
  "FIntEnableCullableScene2HundredthPercent3": "100"
    ...
    getgenv().FourHub_fflag_preset = FFLAG_PRESET;
    getgenv().FourHub_inject_fflags = function(json_str)
        ...
        if writefile then
            pcall(function()
                if makefolder and isfolder and not isfolder("FourHub") then makefolder("FourHub") end;
                writefile("FourHub/fflags.json", json_str);
                saved = true;
            end);
        end;
...
do
    ...
    motion_blur.Name = "FourHub_motion_blur";
    motion_blur.Size = 0;
...
do
    local forced = false;
    if not Library.__FourHubDragHooked and type(Library.MouseIsOverFrame) == "function" then
        Library.__FourHubDragHooked = true;
...
local FourHub_state = { esp_hidden = false, vm_chams_last = 0 };

trove:Add(run_service.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function()
	tickval = tick();

	if not ELI.esp.enable then
		if not FourHub_state.esp_hidden then
			FourHub_state.esp_hidden = true;
			for _, player in players:GetPlayers() do
	...
	FourHub_state.esp_hidden = false;
	for _, player in players:GetPlayers() do
...
do
    FourHub_state.dmgCallbacks = {};
    FourHub_state.dmgQueue = {};
    FourHub_state.onDamage = function(fn) FourHub_state.dmgCallbacks[#FourHub_state.dmgCallbacks + 1] = fn end;
    FourHub_state.fireDamage = function(plr, amount, worldpos)
        local q = FourHub_state.dmgQueue;
    ...
    trove:Add(run_service.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function()
        ...
        for i = 1, n do
            local d = q[i];
            for _, cb in ipairs(FourHub_state.dmgCallbacks) do
...
do
    ...
    FourHub_state.onDamage(function(plr, amount, worldpos)
...
local old_tracer = tracer_effect.Play; tracer_effect.Play = function(...)
    ...
    if ELI.silent_aim.visualize and ELI.silent_aim.enable
        ...
        local st = FourHub_state.silentTarget;
        if st and (tick() - (FourHub_state.silentTargetT or 0)) < 0.2 then
...
do
local vm_material_presets = getgenv().FourHub_materials.presets;
...
trove:Add(run_service.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function()
    ...
    local now = tick();
    if now - FourHub_state.vm_chams_last < 0.1 then return end;
    FourHub_state.vm_chams_last = now;
...
do
    function skybox_changer:init()
        ...
        self.list = {
            ...
            ["FourHub Sky"] = { SkyboxBk = "rbxassetid://1898724755", SkyboxDn = "rbxassetid://1898727189", SkyboxFt = "rbxassetid://1898722814", SkyboxLf = "rbxassetid://1898729298", SkyboxRt = "rbxassetid://1898741025", SkyboxUp = "rbxassetid://1898736761" },
        };
...
local replicate_from_server = local_fighter.ReplicateFromServer; local_fighter.ReplicateFromServer = function(controller, _type, ...)
    if _type == 'DamageNumberEffect' then
        ...
        if FourHub_state.fireDamage and ELI.damage_numbers.enable then
            local hp = players:GetPlayerFromCharacter(hit_character);
            local ok, pos = pcall(function()
                return hit_root.Position + Vector3.new(0, is_headshot and 2.6 or 2.1, 0);
            end);
            FourHub_state.fireDamage(hp, hit_damage, ok and pos or nil);
        end;
    ...
    if ELI.silent_aim.enable and debug.info(3, "n") == "StartShooting" then
        if math.random(1, 100) <= ELI.silent_aim.hit_chance then
            ...
            if closest and closest.Character then
                ...
                if target_pos then
                    FourHub_state.silentTarget = target_pos;
                    FourHub_state.silentTargetT = tick();
                    local args = {...};
                    args[3] = target_pos;
    ...
    local aim_tabs = legit_tab:AddLeftTabbox(); do
        local silentaim_tab = aim_tabs:AddTab('silent aim'); do
            ...
            silentaim_tab:AddToggle('show_fov_enable', {
                ...
                Title = 'fov color',
                Default = Color3.fromRGB(0, 30, 255),
                Callback = function(v)
...
do
    ...
    local function apply(kind) pcall(function() getgenv().FourHub_apply_opt(kind) end) end;
    ...
    ff_box:AddLabel('2 it applies live if your executor supports it and always saves them to the FourHub fflags file', true);
    ...
    do
        ...
        local base = 'open its settings > fastflags editor, paste or import the FourHub fflags file then save';
        ...
        local guide;
        if name == '' then
            guide = 'could not auto detect your executor, open its fastflags or config editor, import the FourHub fflags file then fully rejoin roblox';
        else
    ...
    local function do_inject(json)
        local injector = getgenv().FourHub_inject_fflags;
        ...
        elseif res.saved then
            Library:Notify('fflags saved to FourHub/fflags.json - import in your executor & rejoin', 7);
        else
    ...
    ff_box:AddButton('save preset to file', function()
        ...
        pcall(function()
            if makefolder and isfolder and not isfolder('FourHub') then makefolder('FourHub') end;
            writefile('FourHub/fflags.json', getgenv().FourHub_fflag_preset or '{}');
        end);
        Library:Notify('saved preset to FourHub/fflags.json', 5);
    end);
end;
...
local visuals_tab = Tabs.visualstab; do
    ...
    local viewmodel_thing = visuals_tab:AddLeftTabbox(); do
        local viewmodel_tab = viewmodel_thing:AddTab('viewmodel'); do
            ...
            local vm_material_names = getgenv().FourHub_materials.names;
...
local world_tab = Tabs.worldtab; do
    ...
    shader_bloom.Name = 'FourHub_bloom';
    ...
    shader_sunrays.Name = 'FourHub_sunrays';
...
getgenv().FourHub_cosmetic_state = getgenv().FourHub_cosmetic_state or {
    gui = nil,
    frame = nil,
    inner = nil,
    panel = nil,
...
v555:AddToggle("AnySkinMode", {
    Text = "any skin",
    Default = getgenv().FourHub_cosmetic_state.anyModel or false,
    Callback = function(val)
        getgenv().FourHub_cosmetic_state.anyModel = val and true or false;
        local st = getgenv().FourHub_cosmetic_state;
...
local function openCosmeticChanger(showUi)
    ...
    local SKINS_FOLDER = "FourHub_skins";
    ...
    local uiState = getgenv().FourHub_cosmetic_state;
    ...
    gui.Name = "FourHub_cosmetic_changer";
    ...
    local weaponSkinSeedRaw = {
        ...
        Scythe = {"Scythe of Death", "Anchor", "FourHub Scythe", "Keythe", "Bat Scythe", "Cryo Scythe", "Crystal Scythe", "Glorious Scythe", "Bug Net"},
    ...
    getgenv().FourHub_apply_cosmetic_theme = applyCosmeticUiTheme;
end;
...
;(function()
    local cfg = ELI;
    local material_presets = getgenv().FourHub_materials.presets;
    local material_preset_names = getgenv().FourHub_materials.names;
    ...
    ch_text_gui.Name = "FourHub_crosshair_text";
...
autoload:AddLabel('automatic loading is disabled in this build', true);
autoload:AddButton('clear teleport queue', function()
    pcall(function()
    ...
    Library:Notify('teleport queue cleared', 2);
end);
